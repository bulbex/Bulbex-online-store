...
const mainSliderBtns = document.querySelectorAll(".main_slider_btn")
const sliderImgContainer = document.getElementById("slider_img_container")
const sliderImgs = document.getElementById("slider_img_container").children
...
let switchBack
let sliderInterval
let timingSliderInterval = 8000

window.addEventListener("load", () => setSliderInterval())

function setSliderInterval() {
    sliderInterval = setInterval(sliderFunc, timingSliderInterval)
}

function switchImage(btnId) {
    clearInterval(sliderInterval)
    if (btnId === "forward") {
        switchBack = "-" + Number(currentImgId + 1) + "00%"
        sliderImgContainer.style.left = switchBack
        currentImgId++
        mainSliderBtns[0].style.display = "block"
    } else if (btnId === "back") {
        switchBack = "-" + Number(currentImgId - 1) + "00%"
        sliderImgContainer.style.left = switchBack
        currentImgId--
        mainSliderBtns[1].style.display = "block"
    } else {
        timingSliderInterval = 8000
        sliderImgContainer.style.left = "0"
        mainSliderBtns[0].style.display = "none"
        mainSliderBtns[1].style.display = "block"
        currentImgId = 0
    }
    if (currentImgId === sliderImgs.length - 1) {
        mainSliderBtns[1].style.display = "none"
...
function sliderFunc() {
    timingSliderInterval = 5000
    currentImgId === sliderImgs.length - 1
        ? switchImage("repeat")
        : switchImage("forward")
}
...
mainSliderBtns.forEach((elem) => elem.addEventListener("click", (e) => {
    timingSliderInterval = 8000
    switchImage(e.currentTarget.id)
}))
